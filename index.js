...
module.exports.parsers = {
  astro: {
    parse: (text) => {
      ...
      html = html
        ? { ...html, text: text.slice(html.start, html.end), isRoot: true }
        : null;
...
const findExpressionsInAST = (node, collect = []) => {
  ...
  if (node.children) {
    collect.push(
      ...[].concat(...node.children.map((child) => findExpressionsInAST(child)))
    );
  }
  return collect;
};
...
const formatExpression = (
  { expression: { codeChunks, children } },
  text,
  options
) => {
...
const isAstroScript = (node) =>
  node.type === "concat" &&
  node.parts[0] === "<script" &&
  node.parts[1].type === "indent" &&
  node.parts[1].contents.parts.find((v) => v === "$");
  ...
  const recurse = (node, { parent }) => {
    ...
    if (inAstroScript) {
      if (node.type === "break-parent") {
        parent.parts = parent.parts.filter(
          (part) => !["break-parent", "line"].includes(part.type)
        );
      }
      if (node.type === "indent") {
    ...
    embed(path, print, textToDoc, options) {
      ...
      if (node.type === "Script" && node.context === "setup") {
        return [
          "---",
          hardline,
          textToDoc(node.content, { ...options, parser: "typescript" }),
          "---",
          hardline,
          hardline,
        ];
        ...
        if (expressions.length > 0) {
          const parts = [].concat(
            ...expressions.map((expr, i, all) => {
              const prev = all[i - 1];
              const start = node.text.slice(
                (prev?.end ?? node.start) - node.start,
                expr.start - node.start
              );
              const exprText = formatExpression(
                expr,
                node.text.slice(
                  expr.start - node.start + 1,
                  expr.end - node.start - 1
                ),
                options
              );
