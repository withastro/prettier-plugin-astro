...
`.replaceAll('\n', '\r\n');
	...
	for (const astroCompressHTML of ['jsx', 'html', 'none'] as const) {
		const firstPass = await formatAstro(input, {
			astroCompressHTML,
			endOfLine: 'crlf',
		});
		expect(firstPass).toBe(output);
		expect(await formatAstro(firstPass, { astroCompressHTML, endOfLine: 'crlf' })).toBe(firstPass);
	}
});
...
it('preserves style content when embedded language formatting is disabled', async () => {
	const input = `<style>
  .a{color:red}
    /* untouched */
</style>`;
