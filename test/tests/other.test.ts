...
test('can format an Astro file with a `<!DOCTYPE html>` + embedded JSX expressions', 'doctype-with-embedded-expr');
...
test('can format an Astro file with a JSX expression in an attribute', 'attribute-with-embedded-expr');

test('does not alter html comments', 'html-comment', { mode: 'unaltered' });

test('can format an Astro file with a JSX expression and an HTML Comment', 'expr-and-html-comment');

test('converts valid shorthand variables into shorthand', 'converts-to-shorthand');

// test.failing('an Astro file with an invalidly unclosed tag is still formatted', Prettier, 'unclosed-tag');

test('can format an Astro file with components that are the uppercase version of html elements', 'preserve-tag-case');
...
test('Can format an Astro file with a JS style prettier ignore comment: https://prettier.io/docs/en/ignore.html', 'prettier-ignore-js');
